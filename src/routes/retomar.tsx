...
const buscarSessao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      locale: lead.locale === "es" ? ("es" as const) : ("pt" as const),
      letra: {
        titulo: m.titulo ?? "",
        letra: m.letra,
...
const COPY = {
  pt: { indo: "abrindo a sua letra…", erro: "Não achei essa letra.", erroSub: "O link pode ter vindo cortado. Responda o e-mail que a gente reenvia." },
  es: { indo: "abriendo tu letra…", erro: "No encontré esa letra.", erroSub: "El link pudo venir cortado. Responde el correo y te lo reenviamos." },
} as const;
